...
describe('btc utils', () => {
  test('Should be a VALID BTC address', () => {
    const btcApi = new utils.utxo.btc()
  ...
  test('Should monitor a BTC utxo given an address', async () => {
    ...
    const btcApi = new utils.utxo.btc(network)
    let btcTxBroadcasted = 0
    let btcTxConfirmed = 0

    await btcApi
      .monitorUtxoByAddress(BTC_TESTING_ADDRESS, pollingTime)
      .once('broadcasted', () => {
        btcTxBroadcasted += 1
      })
      .once('confirmed', () => {
  ...
  test('Should get all BTC utxo given an address', async () => {
    const network = 'testnet'
    const btcApi = new utils.utxo.btc(network) as Btc
